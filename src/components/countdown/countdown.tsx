...
import MBDDate from '../../pages/model/MBDDate'
...
const CountdownBlock: FC<{ value: number; unit: React.ReactNode }> = (
    props
) => {
    return (
        <div className='countdown-block-container'>
            <div className='countdown-time'>{props.value}</div>
            <div className='countdown-time-type'>{props.unit}</div>
        </div>
    )
}
